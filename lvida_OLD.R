...
listDags <- allDags(ccomp_m,ccomp_m,NULL) # get all possible DAGs from that pattern
if(is.null(listDags)){
...
###################################################################
###################################################################
lv.ida <- function(x.pos,y.pos,mcov,pag,method="global",nMags=500,
      ...
      if(gMag_x[x.pos,y.pos] != 0) { ### if y is adjacent to x in gMag_x
        beta.hat[i] <- NA
      	       ...
      	       beta.hat[i] <- NA
                    ...
                    if (verbose) cat("Fit - y:",y.pos,"x:",c(x.pos,dsepset),
                         "|b.hat=",beta.hat[i],"\n")
               }
    } ## for ( i  n.dags)
  } ## else : method = "global"
